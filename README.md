...
Built at the UC Berkeley AI Hackathon 2026 as a three-person team; I own the dashboard and the memory/persistence layer.

### [Polaris](https://github.com/V-prajit/Polaris)
An ML platform that turns satellite imagery into a parking-capacity estimate: drop a pin in a city and get stall counts, confidence intervals, and a 0-100 accessibility score in under two seconds. It fuses three independent methods into one weighted, confidence-scored estimate: a SegFormer-b5 model fine-tuned on the 12,617-tile ParkSeg12k dataset, YOLO detectors run under SAHI sliced inference to catch small objects, and geometry-only heuristics from OSM boundaries. Mapped the entirety of Atlanta on H3 hex grids as the demo dataset.
